...
sub factorization{
	...
	my $least_prime = get_least_prime($self, sqrt($quotient));
	print "sqrt(quotient): ". sqrt($quotient) ."\n";
	print "least_prime: $least_prime\n";
	foreach my $prime ( @sorted ){
	    ...
	    print "IS THERE???\n";
	    $least_prime = get_least_prime($self, sqrt($quotient));
	}
    }
    return;
}
